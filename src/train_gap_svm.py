...
from sklearn.svm import SVC # --- Using Support Vector Classifier (SVM) ---
...
# TRAIN FUNCTION (SVM)
# =====================================================
def train_svm_directional(df: pd.DataFrame, direction: str):
    """Trains and evaluates an SVM model for a given trade direction."""

    print(f"\n================ TRAINING SVM ({direction}) ================")
    ...
    # Support Vector Classifier (RBF kernel is often best for financial data)
    model = SVC(
        ...
        random_state=42,
        probability=True             # Essential for ensemble voting
    )
    
    # SVM training is significantly slower than tree models
    ...
    plt.title(f'Confusion Matrix: {direction} (SVM)')
    plt.ylabel('Actual Label')
    plt.xlabel('Predicted Label')
    
    # Save Plot
    plot_filename = f"cm_{direction.lower()}_svm.png"
    ...
    # SAVE MODEL
    # =====================================================
    model_path = f"{MODEL_DIR}/gap_{direction.lower()}_svm.pkl"
    scaler_path = f"{MODEL_DIR}/gap_{direction.lower()}_scaler_svm.pkl"
...
def main():
    ...
    train_svm_directional(df, "Long")
    train_svm_directional(df, "Short")
